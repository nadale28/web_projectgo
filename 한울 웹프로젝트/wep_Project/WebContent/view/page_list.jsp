...
function stop_reg_people(){
  ...
  function go_search(){
	...
	function go_applicant(rcv){
		location.href="applicant.do?num="+rcv;
	}
</script>
...
<jsp:include page="toptop.jsp"/>
	...
	<div class="board_wrap">
		...
		<table id="board_main">
			...
			<% if(session.getAttribute("user_type").equals("c")){ %>
			<td>신청자 목록</td>
			<%} %>
		<%} %>
			</tr>			
		...
		<c:forEach items="${requestScope.notice_bor_list }" var="board">
			<tr class = "txtstyle1">
				...
				<c:if test = "${sessionScope.user_type=='c' && sessionScope.memId!=board.id}" >
		<td >-</td>
	</c:if> 
	<c:if test = "${sessionScope.user_type=='c' && sessionScope.memId==board.id}" >
		<td onclick="go_applicant(${board.num});">조회</td>
...
<%if(session.getAttribute("memId")==null) {%>
		...
		<%}else{ %>
		<div id="btn_write" >
	<c:if test="${sessionScope.user_type!='c'}">
		<span class="hover effect3" onclick="not_company()">글쓰기</span>
	</c:if>
	<script>
		function not_company(){
			alert('단체회원만 글쓰기가 가능합니다.');
		}
	</script>
	...
	<c:if test="${sessionScope.user_type=='c'}">
		...
		<form name="search" method="post" >
			<select id="search_name" name="search_name" style="float:left; height:25px; margin-top:9px;">
			...
			<input type="text" name="search_value" style="display:block; height:20px;float:left; margin-top:9px"/> 
			<img src="img/search.png" id="searchimg" 
			onClick="go_search();" width="25px" height="25px" style="margin-top:9px"/>
			</form>
		</div>
		<br/>
	<br/>
